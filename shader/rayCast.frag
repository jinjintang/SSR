...
{	
	...
	float roughness = 0.1;
	seed = uv;
	vec2 noiseUV = (uv+ ubo.randomVector.xy)*vec2(1280.0/1024.0,720.0/1024.0);
	vec2 Xi = texture(samplerNoise,noiseUV).xy ;//vec2(rand(),rand());
	Xi.y = mix(Xi.y, 0.0,0.7);
	vec4 H = TangentToWorld(viewNormal.xyz, ImportanceSampleGGX(Xi, roughness));;//importanceSample_GGX(Xi, roughness, viewNormal.xyz);
