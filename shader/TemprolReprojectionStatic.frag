...
vec4 temporal_reprojection(vec2 ss_txc, float vs_dist)
	{
	
		vec4 texel0 = sample_color(_MainTex, ss_txc);
	
		vec4 texel1 = sample_color(_PrevTex, ss_txc);
		...
		// output
		return texel1+(texel1-texel0)*k_feedback;
	}
...
{  
	...
	outFragColor = blur(uv);


	/*float vs_dist = LinearizeDepth(c_frag.z);
	
	// temporal resolve
	vec4 color_temporal = temporal_reprojection(ss_txc, vs_dist);
